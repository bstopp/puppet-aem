...
describe 'aem::dispatcher', :type => :class do
  ...
  context 'default parameters' do
    ...
    it do
      ...
      'owner'     => 'root',
      'replace'   => 'true'
      ).with_content(/.*DispatcherConfig\s*\/etc\/httpd\/conf.modules.d\/dispatcher.farms.any/
      ).with_content(/.*DispatcherLog\s*\/var\/log\/httpd\/dispatcher.log/
      ).with_content(/.*DispatcherLogLevel\s*warn/
      ).with_content(/.*DispatcherNoServerHeader\s*off/
      ).with_content(/.*DispatcherDeclineRoot\s*off/
      ).with_content(/.*DispatcherUseProcessedURL\s*off/
      ).with_content(/.*DispatcherPassError\s*0/)
    end

    it { is_expected.to contain_anchor('aem::dispatcher::begin') }
    it { is_expected.to contain_file('/etc/httpd/modules/mod_dispatcher.so').that_requires('File[/etc/httpd/modules/dispatcher-apache2.X-4.1.X.so]') }
    it {
